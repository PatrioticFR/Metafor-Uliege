...
def getMetafor(d={}):
    ...
    fct_TOL.setData(0.0, 1.0)  # Début loading
    fct_TOL.setData(T_load * 0.75, 0.05)  # Fin loading
    fct_TOL.setData(T_load, 0.03)  # Transition
    fct_TOL.setData(T_load + 1.0, 0.01)  # Début stabilisation
    fct_TOL.setData(T_load + 2.5, 0.005)  # Stabilisation stricte
    fct_TOL.setData(T, 1.0)  # Fin relâchement

    # MECHANICAL ITERATION MANAGER - OPTIMIZED avec gestion de compatibilité
    mim = metafor.getMechanicalIterationManager()

    # Ajout de la méthode de calcul résiduel pour plus de robustesse
    try:
        mim.setResidualComputationMethod(Method4ResidualComputation())
        print("Méthode de calcul résiduel améliorée activée")
    except AttributeError:
        print("Méthode de calcul résiduel non disponible - utilisation par défaut")

    # ENHANCED CONVERGENCE CONTROL pour phases critiques
    fct_MaxIter = PieceWiseLinearFunction()
    fct_MaxIter.setData(0.0, 25.0)  # Standard au début
    fct_MaxIter.setData(T_load * 0.75, 35.0)  # Plus d'itérations vers fin loading
    fct_MaxIter.setData(T_load, 40.0)  # Maximum pour transition
    fct_MaxIter.setData(T_load + 2.5, 35.0)  # Stabilisation critique
    fct_MaxIter.setData(T, 25.0)  # Standard jusqu'à la fin

    # Application du contrôle adaptatif des itérations
    try:
        mim.setMaxNbOfIterations(40, fct_MaxIter)  # Fonction adaptative
        print("Contrôle adaptatif des itérations configuré avec succès")
    except (AttributeError, TypeError):
        print("Utilisation du contrôle fixe des itérations")
        mim.setMaxNbOfIterations(40)  # Fallback fixe plus élevé

    # Configuration des tolérances
    mim.setResidualTolerance(5e-5, fct_TOL)
    mim.setPredictorComputationMethod(EXTRAPOLATION_MRUA)  # Meilleure prédiction

    # Méthode de recherche linéaire - avec gestion de compatibilité
    try:
        mim.setLineSearchMethod(LINESEARCH_ARMIJO)  # Recherche linéaire robuste
        print("Méthode de recherche linéaire configurée avec succès")
    except AttributeError:
        print("Méthode de recherche linéaire non disponible - utilisation par défaut")
        # Alternative: essayer d'autres noms possibles
        try:
            mim.setLineSearch(LINESEARCH_ARMIJO)
            print("Recherche linéaire alternative configurée")
        except AttributeError:
            try:
                mim.enableLineSearch(True)
                print("Recherche linéaire basique activée")
            except AttributeError:
                print("Aucune méthode de recherche linéaire disponible - paramètres par défaut")

    # Contrôle des incréments de charge
    try:
        mim.setMaxNbOfLoadIncrements(50)
        print("Contrôle des incréments de charge configuré avec succès")
    except AttributeError:
        print("Contrôle par pas de temps utilisé pour la gestion des incréments")
        try:
            mim.setMaxNumberOfLoadIncrements(50)
            print("Méthode alternative des incréments configurée")
        except AttributeError:
            print("Pas de contrôle d'incréments disponible - utilisation du contrôle temporel uniquement")

    # Paramètres de remaillage automatique pour grandes déformations (si nécessaire)
    try:
        mim.setAutoRemeshingParameters(0.8, 0.2, 2)  # Seuils de qualité
        print("Paramètres de remaillage automatique configurés avec succès")
    except AttributeError:
        print("Remaillage automatique non disponible dans cette version")

    # Paramètres de stabilité additionnels pour améliorer la convergence
    try:
        # Méthode d'accélération de convergence
        mim.setConvergenceAccelerationMethod(CONVERGENCE_AITKEN)
        print("Accélération de convergence Aitken activée")
    except AttributeError:
        print("Accélération de convergence non disponible - utilisation par défaut")
