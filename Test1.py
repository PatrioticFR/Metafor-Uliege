...
class NightSequenceManager:
    """Gestionnaire optimisé pour séquencement nocturne automatique"""

    def __init__(self):
        # Configuration simplifiée pour usage nocturne
        self.base_workspace = r"C:\Users\adpie\Desktop\Stage 4A\Metafor\workspace\Test1"
        self.results_dir = r"C:\Users\adpie\Desktop\Stage 4A\Metafor\AutomatedResults"

        os.makedirs(self.results_dir, exist_ok=True)

        # Log avec timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.log_file = os.path.join(self.results_dir, f"night_sequence_{timestamp}.txt")

        # État du séquencement
        self.configurations = []
        self.current_config_index = 0
        self.total_simulations = 0
        self.start_time = None

        self.log("=== SÉQUENCEMENT NOCTURNE INITIALISÉ ===")

    def log(self, message):
        """Log simplifié avec timestamp"""
        ...
        try:
            ...
            pass  # Continue même si log file échoue

    def define_night_configurations(self):
        """Définir toutes les configurations pour la nuit"""
        configs = []

        # Configurations de base (vous pouvez en ajouter jusqu'à 20)
        base_configs = [
            {
                'name': 'BeCu_Baseline_Thermal',
                'description': 'Be-Cu avec thermique (référence)',
                'params': {
            ...
            },
            {
                ...
                'description': 'Be-Cu mécanique seul',
                'params': {
                    'blade_material': 'BE_CU',
                    'blade_thickness': 0.24,
                    'blade_length': 105.25,
        ...
        self.configurations = base_configs
        self.total_simulations = len(self.configurations)

        self.log(f"Configurations définies : {self.total_simulations}")
        for i, config in enumerate(self.configurations, 1):
            self.log(f"  {i}. {config['name']}")

        return self.configurations

    def get_current_config_params(self):
        """Retourne les paramètres de la configuration actuelle"""
        if 0 <= self.current_config_index < len(self.configurations):
            return self.configurations[self.current_config_index]['params']
        return {}
    ...
    def backup_results(self):
        ...
        config_name = self.configurations[self.current_config_index]['name']
        ...
        try:
            ...
            if os.path.exists(self.base_workspace):
                ...
                self.log(f"Résultats sauvés : {config_name}")

        except Exception as e:
            self.log(f"Erreur sauvegarde {config_name}: {e}")

    def clean_workspace(self):
        """Nettoyage rapide du workspace"""
        try:
            if os.path.exists(self.base_workspace):
                for filename in os.listdir(self.base_workspace):
                    if filename.endswith(('.v', '.ascii')) or filename == 'resFiles.txt':
                        filepath = os.path.join(self.base_workspace, filename)
                        if os.path.isfile(filepath):
                            os.remove(filepath)
        except:
            pass  # Continue même si nettoyage échoue

    def advance_simulation(self):
        """Passer à la simulation suivante"""
        # Sauvegarder la simulation actuelle
        self.backup_results()

        # Nettoyer pour la suivante
        self.clean_workspace()

        # Avancer l'index
        self.current_config_index += 1

        if self.current_config_index < self.total_simulations:
            config = self.configurations[self.current_config_index]
            self.log(f"=== SIMULATION {self.current_config_index + 1}/{self.total_simulations} ===")
            self.log(f"Configuration: {config['name']}")
            return True
        else:
            self.log("=== TOUTES LES SIMULATIONS TERMINÉES ===")
            self.generate_final_report()
            return False

    def generate_final_report(self):
        """Rapport final de la nuit"""
        if self.start_time:
            total_time = datetime.now() - self.start_time
            self.log(f"Temps total écoulé: {total_time}")

        self.log("Résultats disponibles dans:")
        for config in self.configurations:
            result_dir = os.path.join(self.results_dir, config['name'])
            status = "✓" if os.path.exists(result_dir) else "✗"
            self.log(f"  {status} {config['name']}")

    def start_sequence(self):
        """Démarrer la séquence nocturne"""
        self.define_night_configurations()
        self.start_time = datetime.now()

        if self.configurations:
            config = self.configurations[0]
            self.log(f"=== SIMULATION 1/{self.total_simulations} ===")
            self.log(f"Configuration: {config['name']}")
            return True
        return False
...
def get_night_manager():
    """Obtenir le gestionnaire nocturne"""
    global _night_manager
    if _night_manager is None:
        _night_manager = NightSequenceManager()
        _night_manager.start_sequence()
    return _night_manager
...
def getMetafor(d={}):
    ...
    """
    # === INTÉGRATION SÉQUENCEMENT ===
    manager = get_night_manager()
    current_params = manager.get_current_config_params()
    d.update(current_params)  # Applique les paramètres de la config actuelle
    ...
    print(f"\n--- Simulation {manager.current_config_index + 1}/{manager.total_simulations} ---")
    print(f"Config name: {manager.configurations[manager.current_config_index]['name']}")
...
if __name__ == '__main__':
    ...
    while True:
        metafor = getMetafor()
        try:
            metafor.run()  # Exécute la simulation complète
            manager.log("Simulation terminée avec succès.")
        except Exception as e:
            manager.log(f"ERREUR durant simulation : {e}")
            break

        if not manager.advance_simulation():
            manager.log("Toutes les simulations ont été exécutées.")
            try:
                postpro()
                manager.log("postpro() exécuté avec succès.")
            except Exception as e:
                manager.log(f"postpro() échoué : {e}")
            break
