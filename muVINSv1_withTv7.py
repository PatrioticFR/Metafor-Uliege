...
def getMetafor(d={}):
    ...
    if sim_config.enable_thermal:
        ...
        tsm.setNextTime(sim_config.loading_time * 0.25, 125, 0.02)  # Early loading Jusqu'à 2.5s
        tsm.setNextTime(sim_config.loading_time * 0.5, 125, 0.02)  # Mid loading Jusqu'à 5s
        tsm.setNextTime(sim_config.loading_time * 0.75, 250, 0.01)  # Late loading Jusqu'à 7.5s
        tsm.setNextTime(sim_config.loading_time, 250, 0.01)  # Final mechanical phase Jusqu'à 10s

        # Phase 2: Stabilization (T_load to temp_start_time) - MUCH SMALLER STEPS
        # Cette phase est critique pour la convergence
        tsm.setNextTime(sim_config.loading_time + 1.0, 100, 0.01)  # Premier seconde après loading Jusqu'à 11s
        tsm.setNextTime(sim_config.loading_time + 2.5, 150, 0.01)  # Stabilisation intermédiaire Jusqu'à 12.5s
        tsm.setNextTime(sim_config.temp_start_time - 2.0,110 , 0.05)  # Pré-thermal Jusqu'à 18s
        tsm.setNextTime(sim_config.temp_start_time, 40, 0.05)  # Juste avant thermal Jusqu'à 20s
        ...
        tsm.setNextTime(sim_config.temp_start_time + thermal_duration * 0.1, 30, 0.05)  # Start thermal Jusqu'à 21.5s
        tsm.setNextTime(sim_config.temp_start_time + thermal_duration * 0.5, 120, 0.05)  # Mid thermal Jusqu'à 27.5s
        tsm.setNextTime(sim_config.temp_end_time, 375, 0.02)  # End thermal - finest steps Jusqu'à 35s
        ...
        tsm.setNextTime(T_load * 0.5, 250, 0.02)
        tsm.setNextTime(T_load,500, 0.01)
        tsm.setNextTime(T, 400, 0.05)
    ...
    fct_TOL.setData(sim_config.loading_time + 1.0, 0.02)  # Très strict stabilisation
    fct_TOL.setData(sim_config.loading_time + 2.5, 0.01)  # Maximum stricte 7.5-12s
...
def postpro():
    ...
    # Load the last archive (final state after thermal loading)
    ...
    lanczos.setNumberOfEigenValues(5)  # More modes for better analysis
    lanczos.setSpectralShifting(0.0)
    lanczos.setComputeEigenVectors(True)
    lanczos.setWriteMatrix2Matlab(True)

    print('\n=== MODAL ANALYSIS ON THERMALLY LOADED STRUCTURE ===')
    lanczos.execute()
    lanczos.writeTSC()

    fExtr = FrequencyAnalysisValueExtractor(lanczos)
    curves.add(11, FrequencyAnalysisValueExtractor(lanczos), 'freqs_thermal_state')

    # Extract eigenvalues
    print('\nComputing Eigenvalues after thermal loading...')
    ...
    for i in range(5):
        ...
        win.update()
        print(f'Mode {i + 1}: Eigenvalue = {eigenval:.6e}, Frequency = {frequency_hz:.2f} Hz')
        input("Press enter to continue to next mode...")

    # Read and analyze frequency data
    try:
        with open('freqs_thermal_state.ascii') as f:
            txt = f.readlines()
        freq_values = [float(v) for v in txt[0].strip().split()]
        print(f'\nThermal state eigenvalues = {freq_values}')

        # Convert to frequencies in Hz
        frequencies_hz = [math.sqrt(abs(val)) / (2 * math.pi) for val in freq_values if val > 0]
        print(f'Natural frequencies after thermal loading = {[f"{f:.2f} Hz" for f in frequencies_hz]}')
    except:
        print("Could not read frequency file")
