...
def getMetafor(d={}):
    ...
    if sim_config.enable_thermal:
        # Blade material - Be-Cu with temperature dependence
        materials.define(1, TmElastHypoMaterial)  # Use elastic material instead of plastic for better convergence
        materials(1).put(MASS_DENSITY, 8.36e-9)
        materials(1).put(ELASTIC_MODULUS, 1.0)  # Base value, will be multiplied by function
        materials(1).depend(ELASTIC_MODULUS, fctE, Field1D(TO, RE))
        materials(1).put(POISSON_RATIO, 0.285)
        materials(1).put(THERM_EXPANSION, 1.0)  # Base value, will be multiplied by function
        ...
        # Structure material - Steel (no temperature dependence for rod)
        ...
        materials(2).put(THERM_EXPANSION, 0.0)  # No thermal expansion for rod
        ...
        materials.define(2, ElastHypoMaterial)
        materials(2).put(MASS_DENSITY, 7.93e-9)
        materials(2).put(ELASTIC_MODULUS, 210e3)
        materials(2).put(POISSON_RATIO, 0.3)
    ...
    if sim_config.enable_thermal:
        ...
        ti.setAlpha(-0.3)  # Optimal damping
        metafor.setTimeIntegration(ti)
    ...
    mim.setMaxNbOfIterations(25)  # Increased for better convergence
...
def postpro():
    ...
    print(f"2. Large temperature change (20°C → -253°C)")
