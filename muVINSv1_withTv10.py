...
class BladeConfig:
    """Configuration parameters for the blade with automatic compensation"""
    ...
    def __init__(self):
        ...
        self.length = 94.80  # current length (L)
        self.width = 19.1  # current width
    ...
    def get_compensated_clamping_position(self):
        """
        Calculate compensated clamping position based on TFE equations
        Returns (Dx_compensated, Dy_compensated) for maintaining equilibrium
        """
        # Reference values from TFE
        Dx_ref = -67.5227  # reference horizontal location
        Dy_ref = 0.0  # reference vertical location
        enc_ref = 57.32  # reference clamping point location

        # Length compensation (from TFE: f0 evolves as L^(-3/2))
        # When L changes, the stiffness changes as L^(-3), affecting equilibrium
        length_ratio = self.length / self.length_ref

        # Thickness compensation (from TFE: stiffness proportional to h^3/L^3)
        thickness_ratio = self.thickness / self.thickness_ref

        # Width compensation affects mass distribution
        width_ratio = self.width / self.width_ref

        # Stiffness ratio calculation (from beam theory: k ∝ E*h^3*w/L^3)
        stiffness_ratio = (thickness_ratio ** 3) * width_ratio / (length_ratio ** 3)

        # From TFE Figure 4.8-4.9: locus follows 3rd order polynomial
        # dx compensation based on stiffness change
        if stiffness_ratio < 1.0:
            # Lower stiffness requires moving clamp closer to center (more positive dx)
            dx_compensation = Dx_ref * (1.0 - sqrt(1.0 / stiffness_ratio - 1.0) * 0.1)
        else:
            # Higher stiffness allows moving clamp further (more negative dx)
            dx_compensation = Dx_ref * sqrt(stiffness_ratio)

        # dy compensation for length changes (from TFE: longer blades need downward shift)
        # Based on TFE Figure 4.11: downward motion decreases f0
        dy_compensation = Dy_ref - (length_ratio - 1.0) * 5.0  # empirical from TFE curves

        return dx_compensation, dy_compensation

    def get_adjusted_enc_position(self):
        """
        Calculate adjusted enc position to maintain system balance
        Based on MATLAB geometry equations and TFE analysis
        """
        enc_ref = 57.32

        # From MATLAB: CMx - enc = d_enc relationship
        # When blade properties change, enc must adjust to maintain CMx
        length_ratio = self.length / self.length_ref
        thickness_ratio = self.thickness / self.thickness_ref
        width_ratio = self.width / self.width_ref

        # Mass distribution change affects center of mass
        mass_ratio = thickness_ratio * width_ratio * length_ratio

        # Adjust enc based on mass distribution change
        enc_adjusted = enc_ref * (1.0 + (mass_ratio - 1.0) * 0.05)
    ...
    def validate_parameters(self):
        ...
        Based on TFE constraints and physical limitations
        """
        warnings = []

        # Check thickness limits (from TFE: mesh convergence requirements)
        ...
        # Check length limits (from TFE Figure 4.6)
        if self.length < 50.0:
        ...
        # Check aspect ratio (L/h should be reasonable for beam theory)
...
class SimulationConfig:
    ...
    def __init__(self, blade_config):
        ...
        # Clamping parameters
        self.Dx, self.Dy = blade_config.get_compensated_clamping_position() # reference horizontal location and vertical shift
        self.Dx1 = 0.0  # horizontal shift
        self.angleClamp = 0.0  # clamping angle

        self.enc = blade_config.get_adjusted_enc_position()         # Adjusted enc position


def setup_compensated_blade_system(user_overrides=None):
    """
    Setup blade system with automatic parameter compensation

    Args:
        user_overrides: dict with keys like 'blade_thickness', 'blade_length', 'blade_width'

    Returns:
        blade_config, sim_config, validation_warnings
    """
    ...
    if user_overrides:
        if 'blade_thickness' in user_overrides:
            blade_config.thickness = user_overrides['blade_thickness']
        if 'blade_length' in user_overrides:
            blade_config.length = user_overrides['blade_length']
        if 'blade_width' in user_overrides:
            blade_config.width = user_overrides['blade_width']
        if 'blade_material' in user_overrides:
            blade_config.material = user_overrides['blade_material']

    # Create simulation config with compensated parameters
    ...
    print(f"=== COMPENSATED BLADE CONFIGURATION ===")
    print(
        f"Blade Dimensions: L={blade_config.length:.2f}mm, h={blade_config.thickness:.2f}mm, w={blade_config.width:.1f}mm")
    print(f"Material: {blade_config.material}")
    print(f"Compensated Clamping: Dx={sim_config.Dx:.3f}mm, Dy={sim_config.Dy:.3f}mm")
    print(f"Adjusted enc position: {sim_config.enc:.2f}mm")
    print(f"Mesh: {blade_config.get_elements_length()} × {blade_config.get_elements_thickness()} elements")
    print(f"Estimated f0: {blade_config.estimate_resonance_frequency():.2f} Hz")
...
def getMetafor(d={}):
    ...
    blade_config, sim_config, warnings = setup_compensated_blade_system(d)
    ...
    Dx = sim_config.Dx  # Compensated Dx
    Dy = sim_config.Dy  # Compensated Dy
    enc = sim_config.enc # Leaf-spring clamping point location on the rod
