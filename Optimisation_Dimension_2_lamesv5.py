...
class DualBladeConfig:
    ...
    def __init__(self, use_strict_criteria=True):
        ...
        self.inner_material = 'INVAR_CW'  # Modification ici
    ...
    def evaluate_configuration_quality(self, config: Dict) -> float:
        """Évalue la qualité avec critères thermiques"""
        # Évaluation de la stabilité thermique
        thermal_metrics = self.evaluate_thermal_stability(config)

        # Facteurs de pondération modifiés
        w_thermal_stability = 0.4  # Nouveau : stabilité thermique
        w_mean_displacement = 0.3  # Nouveau : centrage sur 0mm
        w_displacement = 0.2  # Réduit
        w_manufacturability = 0.1

        # Score de stabilité thermique
        thermal_stability_score = max(0, 1 - thermal_metrics['thermal_variation'] / 1.0)

        # Score de centrage (meilleur si proche de 0)
        mean_displacement_score = max(0, 1 - abs(thermal_metrics['mean_displacement']) / 1.0)

        # Score de déplacement (inchangé)
        ...
        # Score de manufacturabilité (inchangé)
        ...
        # Score composite
        total_score = (w_thermal_stability * thermal_stability_score +
                       w_mean_displacement * mean_displacement_score +
                       w_displacement * displacement_score +
                       w_manufacturability * manufacturability_score)

        # Ajout des métriques thermiques au config
    ...
    def _create_dual_config_dict(self, w_outer: float, thickness_outer: float, L_outer: float,
        ...
        # Ajout des métriques thermiques
        thermal_metrics = self.evaluate_thermal_stability(config)
        config.update(thermal_metrics)
    ...
    def print_dual_configurations(self, configs: List[Dict], title: str, show_all: bool = False):
        ...
        print(
            "Rank\tL_Out\te_Out\tw_Out\tL_In\tei_In\tw_In\tDisp_Est\tMean_Disp\tTh_Var\tStab_Score\tQuality")
        print("-" * 120)
        ...
        for i, config in enumerate(configs[:max_display], 1):
            print(f"{i:2d}\t{config['L_outer']:.2f}\t{config['thickness_outer']:.3f}\t{config['w_outer']:.1f}\t"
                  f"{config['L_inner']:.2f}\t{config['thickness_inner']:.3f}\t{config['w_inner']:.1f}\t"
                  f"{config['displacement_est']:.3f}\t{config.get('mean_displacement', 0):.3f}\t"
                  f"{config.get('thermal_variation', 0):.3f}\t{config.get('stability_score', 0):.3f}\t"
                  f"{config.get('quality_score', 0):.3f}")
