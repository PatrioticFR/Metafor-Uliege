...
class DualBladeConfig:
    ...
    def calculate_thermal_compensation_displacement(self, w_outer, thickness_outer, w_inner, thickness_inner,
                                               temp_start=10+273.15, temp_end=50+273.15, n_points=21):
        """
        Calcule le déplacement avec compensation thermique améliorée
        """
        ...
        for temp in temperatures:
            ...
            CTE_outer = self.CTE_becu_function.evaluate(temp)
            CTE_inner = self.CTE_invar_cw_function.evaluate(temp)

            # Propriétés à la température de référence
            E_outer_ref = self.E_becu_function.evaluate(temp_ref)
            E_inner_ref = self.E_invar_cw_function.evaluate(temp_ref)

            # Calcul des géométries
            L_inner = self._calculate_L_inner_from_geometry(thickness_outer, thickness_inner)

            # Rigidités à la température actuelle
            ...
            # Rigidités à la température de référence
            k_outer_ref = self._calculate_k_single_blade(E_outer_ref, w_outer, thickness_outer, self.length)
            k_inner_ref = self._calculate_k_single_blade(E_inner_ref, w_inner, thickness_inner, L_inner)

            # Variation de rigidité due à la température
            delta_k_outer = k_outer - k_outer_ref
            delta_k_inner = k_inner - k_inner_ref

            # Déformation thermique différentielle
            delta_T = temp - temp_ref
            thermal_strain_outer = CTE_outer * delta_T
            thermal_strain_inner = CTE_inner * delta_T

            # Modèle de compensation : les deux lames travaillent en opposition
            # Déplacement dû à la variation de rigidité
            displacement_stiffness = self._calculate_displacement_from_stiffness_change(
                delta_k_outer, delta_k_inner, k_outer_ref, k_inner_ref)

            # Déplacement dû à la dilatation thermique
            displacement_thermal = self._calculate_displacement_from_thermal_expansion(
                thermal_strain_outer, thermal_strain_inner, L_inner)

            # Déplacement total (base + corrections)
            base_displacement = self._calculate_displacement_estimate(w_outer, thickness_outer, w_inner, thickness_inner)
            total_displacement = base_displacement + displacement_stiffness + displacement_thermal

            displacements.append(total_displacement)

        return temperatures, displacements

    def _calculate_displacement_from_stiffness_change(self, delta_k_outer, delta_k_inner, k_outer_ref, k_inner_ref):
        """
        Calcule le déplacement dû à la variation de rigidité
        """
        # Force appliquée (supposée constante)
        F_applied = 1.0  # N (à adapter selon votre système)

        # Rigidité totale de référence
        k_total_ref = k_outer_ref + k_inner_ref

        # Nouvelle rigidité totale
        k_total_new = k_outer_ref + delta_k_outer + k_inner_ref + delta_k_inner

        # Déplacement dû à la variation de rigidité
        if k_total_ref > 0 and k_total_new > 0:
            displacement_ref = F_applied / k_total_ref
            displacement_new = F_applied / k_total_new
            return (displacement_new - displacement_ref) * 1000  # mm
        return 0

    def _calculate_displacement_from_thermal_expansion(self, thermal_strain_outer, thermal_strain_inner, L_inner):
        """
        Calcule le déplacement dû à la dilatation thermique différentielle
        """
        # Dilatation différentielle entre les deux lames
        expansion_outer = thermal_strain_outer * self.length
        expansion_inner = thermal_strain_inner * L_inner

        # Compensation : les lames travaillent en opposition
        # (formule à adapter selon votre géométrie exacte)
        net_expansion = (expansion_outer - expansion_inner) / 2

        return net_expansion * 1000  # mm
    ...
    def evaluate_thermal_stability(self, config):
        ...
        try:
            ...
            print(f"Erreur dans evaluate_thermal_stability: {e}")
            return {
                'thermal_variation': 1.0,
