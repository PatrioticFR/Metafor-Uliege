...
def getMetafor(d={}):
    ...
    R = L / math.pi
    pa1 = pointset.define(23, enc + L - R, 0.0)
    pa2 = pointset.define(24, enc + L - R, 0.0, 1.0)
    axe1 = Axe(pa1, pa2)
    axe1.setSymZ1(1.0)

    Dy = -R / 2  # ou -L/(2*pi)
    pa3 = pointset.define(31, enc, Dy, 0.0)
    pa4 = pointset.define(32, enc, Dy, 1.0)
    axe2 = Axe(pa3, pa4)

    # Fonctions de déplacement de l'axe - CORRIGÉES
    # L'axe ne bouge pas car il est déjà au bon endroit pour la rotation
    fctX = PieceWiseLinearFunction()
    fctX.setData(0.0, 0.0)
    fctX.setData(T_load / 8, 0.0)
    fctX.setData(T_load / 2, 0.0)  # Pas de déplacement X de l'axe
    fctX.setData(3 * T_load / 4, 0.0)
    fctX.setData(T_load, 0.0)
    fctX.setData(T, 0.0)

    fctY = PieceWiseLinearFunction()
    fctY.setData(0.0, 0.0)
    fctY.setData(T_load / 2, 0.0)
    fctY.setData(3 * T_load / 4, 0.0)  # Pas de déplacement Y de l'axe
    fctY.setData(T_load, 0.0)
    fctY.setData(T, 0.0)

    # Appliquer les déplacements (ici, pas de déplacement nécessaire)
    domain.getLoadingSet().define(pa1, Field1D(TX, RE), 0.0, fctX)
    domain.getLoadingSet().define(pa2, Field1D(TX, RE), 0.0, fctX)
    domain.getLoadingSet().define(pa1, Field1D(TY, RE), 0.0, fctY)
    domain.getLoadingSet().define(pa2, Field1D(TY, RE), 0.0, fctY)

    # Rotation - CORRIGÉE
    # Pour un demi-cercle parfait de la position horizontale vers la position finale
    # Angle positif pour tourner dans le sens anti-horaire
    ...
    # Rotation corrigée : +180° pour un demi-cercle parfait
    domain.getLoadingSet().defineRot2(c1, Field3D(TXTYTZ, RE), axe2, angleClamp, fctR2, axe1, -180, fctR, False)

    # Vérification géométrique
    print(f"Position initiale bout de lame : ({enc + L}, 0)")
    print(f"Position finale souhaitée : ({enc}, {Dy})")
    print(f"Distance parcourue : {math.sqrt(L ** 2 + Dy ** 2)}")
    print(f"Longueur demi-cercle théorique : {math.pi * R}")
    print(f"Cohérence géométrique : {abs(math.sqrt(L ** 2 + Dy ** 2) - math.pi * R) < 1e-6}")
    ...
    materials(3).put(PEN_NORMALE,   1e1)
