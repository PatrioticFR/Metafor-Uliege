...
class BladeConfig:
    ...
    def __init__(self):
        ...
        self.inner_length = 102.0 # Calculated automatically after based on the outter blade length so useless here
    ...
    def get_invar_geometry(self, enc, Dx):
        ...
        R_invar = rayon_interne_BeCu - offset - ei / 2
...
def getMetafor(d={}):
    ...
    # Invar blade (inner blade)
    ei = blade_config.inner_thickness
    Li = blade_config.inner_length
    offset = blade_config.inner_offset

    inner_radius_BeCu = enc / 2
    R_beCu =  inner_radius_BeCu + e / 2
    R_invar =  inner_radius_BeCu - offset - ei / 2
    ratio = R_invar / R_beCu
    Li = L * ratio
    Dx_invar = Dx * ratio
    ...
    if blade_config.inner_material == 'BE_CU':
        laws.define(1, LinearIsotropicHardening)
        laws(1).put(IH_SIGEL, 1000.0)  # Approximate elastic limit for BE-CU (~1000 MPa)
        laws(1).put(IH_H, 1000.0)  # Hardening modulus BE-CU (moderate value)
        inner_yield_num = 1
