...
def getMetafor(d={}):
    ...
    # Define radius for semi-circle
    R = L / math.pi  # Radius to achieve perfect semi-circle
    Dy = -R / 2  # Final vertical position of clamping point

    # Rotation axis for the blade to form a semi-circle
    pa1 = pointset.define(23, enc + L / 2, 0.0)  # Center of semi-circle at t=0
    pa2 = pointset.define(24, enc + L / 2, 0.0, 1.0)
    axe1 = Axe(pa1, pa2)
    axe1.setSymZ1(1.0)

    # Clamping point axis
    pa3 = pointset.define(31, enc, Dy, 0.0)  # Final position of clamping point
    pa4 = pointset.define(32, enc, Dy, 1.0)
    axe2 = Axe(pa3, pa4)

    # Axis displacement to move rotation axis from (enc + L/2, 0) to (enc, -Dy)
    fctX = PieceWiseLinearFunction()
    fctX.setData(0.0, 0.0)
    fctX.setData(T_load / 8, 0.0)
    fctX.setData(T_load / 2, 0.5)  # Gradual movement
    ...
    # Apply displacement to rotation axis
    domain.getLoadingSet().define(pa1, Field1D(TX, RE), -(L / 2), fctX)  # Move from enc + L/2 to enc
    domain.getLoadingSet().define(pa2, Field1D(TX, RE), -(L / 2), fctX)
    domain.getLoadingSet().define(pa1, Field1D(TY, RE), Dy, fctY)  # Move to -Dy
    ...
    fctR2.setData(T_load, 0.0)  # No additional rotation needed
    fctR2.setData(T, 0.0)

    # Apply rotation to curve c1 (blade’s right end) to form semi-circle
    domain.getLoadingSet().defineRot2(c1, Field3D(TXTYTZ, RE), axe2, 0.0, fctR2, axe1, 180, fctR, False)
